...
enum WorkoutPlanner {
	...
	static func plan(for station: NAPFAStation, info: data) -> [WorkoutStep] {
		...
		return plan(for: station, previousGrade: previous, targetGrade: target, age: info.Age, sex: info.Gender)
	}
	
	static func plan(for station: NAPFAStation, previousGrade: String, targetGrade: String, age: Int? = nil, sex: Bool? = nil) -> [WorkoutStep] {
		let level = intensity(previous: previousGrade, target: targetGrade)
		let m = level.multiplier
		let stationName = station.displayName(age: age, sex: sex)
