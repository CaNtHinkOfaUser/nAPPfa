...
enum AppKeys {
	...
	static let completedStationsTodayDate = "completedStationsTodayDate"
	static let completedStationsToday = "completedStationsToday"
...
enum AppState {
	...
	static func completedStationsForToday(now: Date = Date()) -> [String] {
		let defaults = UserDefaults.standard
		if let storedDate = defaults.object(forKey: AppKeys.completedStationsTodayDate) as? Date,
		   Calendar.current.isDate(storedDate, inSameDayAs: now) {
			return defaults.object(forKey: AppKeys.completedStationsToday) as? [String] ?? []
		}
		return []
	}
	
	static func recordStationCompletedToday(station: String, now: Date = Date()) {
		let defaults = UserDefaults.standard
		var stations = completedStationsForToday(now: now)
		if !stations.contains(station) {
			stations.append(station)
			defaults.set(stations, forKey: AppKeys.completedStationsToday)
			defaults.set(now, forKey: AppKeys.completedStationsTodayDate)
		}
	}
	...
	static func recordWorkoutCompleted(station: String, now: Date = Date()) {
		recordStationCompletedToday(station: station, now: now)
		let defaults = UserDefaults.standard
	...
	static func isDailyWorkoutComplete(info: data, now: Date = Date()) -> Bool {
		guard hasWorkedOutToday(now: now) else { return false }
		let enabled = enabledGoalStations(from: info).map { $0.rawValue }
		let done = completedStationsForToday(now: now)
		guard !enabled.isEmpty else { return true }
		return enabled.allSatisfy { done.contains($0) }
	}
